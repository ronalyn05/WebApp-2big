                ...
                if (data != null)
                {
                    ...
                    foreach (KeyValuePair<string, Model.subscriptionLogs> entry in subscribed)
                    {
                        if (entry.Value.userIdnum == clientID)
                        {
                            ...
                            clientsTable.Rows.Add(entry.Value.logsId, entry.Value.userFullname, entry.Value.userActivity, entry.Value.activityTime, entry.Value.total, admin.subStatus, admin.currentSubscription);
